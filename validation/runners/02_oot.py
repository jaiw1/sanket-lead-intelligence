...
-------------------------
Train on earlier snapshot months, test on later, embargoing training rows
whose 14-day conversion window would cross the cut. Report precision@10% on
both and the degradation in percentage points.
...
def run(criteria: list[Criterion], ctx: RunnerContext) -> list[Result]:
    ...
    raise NotImplementedError(
        "runner 02 pending: needs data/liability_book.csv (event_month), "
        "data/customer_panel.csv (month), data/application_journeys.csv "
        "(start_ts, disburse_ts)"
    )
