...
window, on 10 quantile bins fixed on the training window. The per-feature
CSI table is emitted alongside for the report even though only PSI gates.
...
def run(criteria: list[Criterion], ctx: RunnerContext) -> list[Result]:
    ...
    raise NotImplementedError(
        "runner 06 pending: needs data/customer_panel.csv (month, all feature "
        "columns), app/public/sanket_data.json (queue[].score)"
    )
