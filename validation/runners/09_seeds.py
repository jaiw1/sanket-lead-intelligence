...
criteria.yaml (>= 5), and report the per-seed precision@10%, the cross-seed
percentile interval and its width in percentage points. Every seed run
appears in report.json; none may be dropped.
...
def run(criteria: list[Criterion], ctx: RunnerContext) -> list[Result]:
    ...
    raise NotImplementedError(
        "runner 09 pending: needs data/liability_book.csv and "
        "data/application_journeys.csv regenerated once per registered seed "
        "(src/make_book.py, src/make_journeys.py with --seed)"
    )
